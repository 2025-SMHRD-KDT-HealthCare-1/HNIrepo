...
const basic = () => {
  ...
  // jsx 문법 특징
  // 하나의 파일안에 html 문법과 js문법을 동시에 작성 가능
  // => 가독성이 좋고, 작성이 쉬움

  // 여러개의 요소가 존재한다면 반드시 하나의 부모요소로 감싸줄 것
  // => div태그가 아니어도(비어있는 태그)가능 <> </>

  // 기존 class속성 대신 className속성 사용
  // 태그는 소문자로 작성 , 대문자는 컴포넌트로 인식
  // 이벤트 타입 문법이 다름 => 기존 js : onclick, React : onClick

  // 표현식 사용 가능
  //  return 부분에서 변수 사용시 {} 활용
  // js문법을 사용시 {}활용 => if 문은 불가
  ...
  return (
    <div>
      ...
      {/* 조건식이 두개일 때 - 삼항 연산자 이용*/}
      {name === '상권' ? <p>정답</p> : <p>거짓</p>}
