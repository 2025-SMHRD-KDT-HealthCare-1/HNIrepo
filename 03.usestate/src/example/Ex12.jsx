...
const Ex12 = () => {
    ...
    // 3번 위치 ComponentDidMount 시점 (useEffect 훅 사용)에서 API Call사용
    useEffect(() => {
        /* 
            비동기 통신을 통해 데이터를 가져오자
            => 생성자 위치(1번 위치), 렌더링 위치(2번위치)에서는 가져오지 않는다
            => useEffect 내에서 데이터를 가져온다
            => Event위치에서도 데이터를 가져올 수 있다(onClick 등) 
        ...
        const key = '8e270320074a8405798dbd28cea3cc9c'
        ...
        //    : axios 라이브러리를 사용하기 위해서 import 필수
        axios.get(url, {
            params : {
                key, targetDt
            }
        ...
        }).catch((err)=>console.error(err)) // .catch : 에러 내용 띄어줌
    },[])
